...
namespace ToolBox
{
	namespace Math
	{
		...
		public:
			...
			static Vector2<T> Lerp(const Vector2<T>& aFrom, const Vector2<T>& aTo, const T& aLerpFactor);
			static Vector2<T> Slerp(const Vector2<T>& aFrom, const Vector2<T>& aTo, const T& aLerpFactor);
		...
		template <class T>
		Vector2<T> Vector2<T>::Lerp(const Vector2<T>& aFrom, const Vector2<T>& aTo, const T& aLerpFactor)
		{
			Vector2<T> result;
			result.x = aFrom.x + (aTo.x - aFrom.x) * aLerpFactor;
			result.y = aFrom.y + (aTo.y - aFrom.y) * aLerpFactor;
			return result;
		}

		template <class T>
		Vector2<T> Vector2<T>::Slerp(const Vector2<T>& aFrom, const Vector2<T>& aTo, const T& aLerpFactor)
		{
			Vector2<T> result;
			T dot = aFrom.Dot(aTo);
			T angle = acos(dot);
			T sinAngle = sin(angle);
			T factor = sin(angle * (1 - aLerpFactor)) / sinAngle;
			result.x = aFrom.x * factor + aTo.x * aLerpFactor;
			result.y = aFrom.y * factor + aTo.y * aLerpFactor;
			return result;
		}
