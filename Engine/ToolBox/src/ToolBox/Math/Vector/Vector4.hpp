...
namespace ToolBox
{
	namespace Math
	{
		...
		public:
			...
			static Vector4<T> Lerp(const Vector4<T>& aFrom, const Vector4<T>& aTo, const T& aLerpFactor);
			static Vector4<T> Slerp(const Vector4<T>& aFrom, const Vector4<T>& aTo, const T& aLerpFactor);
		...
		template <class T>
		Vector4<T> Vector4<T>::Lerp(const Vector4<T>& aFrom, const Vector4<T>& aTo, const T& aLerpFactor)
		{
			return Vector4<T>(aFrom.x + (aTo.x - aFrom.x) * aLerpFactor,
				aFrom.y + (aTo.y - aFrom.y) * aLerpFactor,
				aFrom.z + (aTo.z - aFrom.z) * aLerpFactor,
				aFrom.w + (aTo.w - aFrom.w) * aLerpFactor);
		}

		template <class T>
		Vector4<T> Vector4<T>::Slerp(const Vector4<T>& aFrom, const Vector4<T>& aTo, const T& aLerpFactor)
		{
			T angle = acos(aFrom.Dot(aTo));
			T t = angle * aLerpFactor;
			T invSinAngle = 1 / sin(angle);
			T scale0 = sin(t) * invSinAngle;
			T scale1 = sin(angle - t) * invSinAngle;
			return aFrom * scale0 + aTo * scale1;
		}
